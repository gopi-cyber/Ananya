...
_PROCESS_NAMES = {
    ...
    "google chrome":      {"Windows": "chrome.exe",                  "Darwin": "Google Chrome",        "Linux": "chrome"},
    ...
    "safari":             {"Windows": "msedge.exe",                  "Darwin": "Safari",               "Linux": "safari"},
    "opera":              {"Windows": "opera.exe",                   "Darwin": "Opera",                "Linux": "opera"},
    "whatsapp":           {"Windows": "WhatsApp.Root.exe",             "Darwin": "WhatsApp",             "Linux": "whatsapp"},
    ...
    "teams":              {"Windows": "ms-teams.exe",                "Darwin": "Microsoft Teams",      "Linux": "teams"},
    "microsoft teams":    {"Windows": "ms-teams.exe",                "Darwin": "Microsoft Teams",      "Linux": "teams"},
    "spotify":            {"Windows": "Spotify.exe",                 "Darwin": "Spotify",              "Linux": "spotify"},
    "vlc":                {"Windows": "vlc.exe",                     "Darwin": "VLC",                  "Linux": "vlc"},
    "netflix":            {"Windows": "Netflix.exe",                 "Darwin": "Netflix",              "Linux": "netflix"},
    ...
    "terminal":           {"Windows": "WindowsTerminal.exe",         "Darwin": "Terminal",             "Linux": "gnome-terminal"},
    ...
    "calculator":         {"Windows": "CalculatorApp.exe",            "Darwin": "Calculator",           "Linux": "gnome-calculator"},
    ...
    "epic games":         {"Windows": "EpicGamesLauncher.exe",       "Darwin": "Epic Games Launcher",  "Linux": "legendary"},
}

def close_app_by_name(app_name: str) -> str:
    if not app_name:
        return "No application name provided."
    
    app_lower = app_name.lower().strip()
    resolved_name = app_lower
    
    # Check alias mapping
    if app_lower in _PROCESS_NAMES:
        resolved_name = _PROCESS_NAMES[app_lower].get(_OS, app_lower)
    else:
        for k, v in _PROCESS_NAMES.items():
            if k in app_lower or app_lower in k:
                resolved_name = v.get(_OS, app_lower)
                break
                
    if _OS == "Windows":
        proc_to_kill = resolved_name
        if not proc_to_kill.endswith(".exe") and not proc_to_kill.startswith("ms-settings"):
            proc_to_kill += ".exe"
            
        try:
            # 1. Graceful window close request
            cmd = ["taskkill", "/im", proc_to_kill]
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
            if "SUCCESS" in res.stdout or res.returncode == 0:
                return f"Closed {app_name} gracefully."
            
            # 2. Force terminate if graceful request was ignored or unsuccessful
            cmd_f = ["taskkill", "/f", "/im", proc_to_kill]
            res_f = subprocess.run(cmd_f, capture_output=True, text=True, timeout=5)
            if "SUCCESS" in res_f.stdout or res_f.returncode == 0:
                return f"Closed {app_name}."
        except Exception as e:
            print(f"[Settings] Windows taskkill failed: {e}")
        ...
        try:
            ...
            pass
        try:
            subprocess.run(["killall", resolved_name], capture_output=True, timeout=5)
            return f"Closed {app_name}."
        except Exception:
            pass
        ...
        try:
            subprocess.run(["pkill", "-f", resolved_name], capture_output=True, timeout=5)
            return f"Closed {app_name}."
        except Exception:
            pass

    # 3. psutil fallback for maximum robust process matching
    try:
        import psutil
        closed = False
        target_pat = resolved_name.lower().replace(".exe", "")
        for proc in psutil.process_iter(['pid', 'name']):
            try:
                pname = (proc.info['name'] or "").lower().replace(".exe", "")
                if target_pat in pname or pname in target_pat or app_lower in pname:
                    proc.terminate()
                    closed = True
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue
        if closed:
            return f"Closed {app_name} processes."
    except ImportError:
        pass

    return f"Attempted to close {app_name}, but could not find its running process."
...
def computer_settings(
    ...
    if action == "close_app":
        ...
        try:
            ...
            return f"Action failed (close_app): {e}"
