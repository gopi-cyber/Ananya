...
_SYSTEM_PROMPT = (
    "You are Ananya, a helpful and precise female AI assistant. "
...
def _capture_camera(player=None) -> tuple[bytes, str]:
    # Fallback to capturing the screen where the native Windows Camera App is active
    print("[Vision] 🖥️ Capturing screen with native Windows Camera App active...")
...
class _VisionSession:
    ...
    def abort_playback(self):
        ...
        print("[Vision] 🛑 Playback buffer cleared instantly on abort!")
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Ananya: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
        ...
        def playback_callback(outdata, frames, time_info, status):
            ...
            with self.audio_playback_lock:
                if len(self.audio_playback_buffer) >= bytes_needed:
                    outdata[:bytes_needed] = self.audio_playback_buffer[:bytes_needed]
                    del self.audio_playback_buffer[:bytes_needed]
                else:
                    present = len(self.audio_playback_buffer)
                    if present > 0:
                        outdata[:present] = self.audio_playback_buffer[:present]
                        self.audio_playback_buffer.clear()
...
def screen_process(
    ...
    # Automatically open the native Camera app if the user asks a camera query
    if angle == "camera" and player and getattr(player, "dashboard", None) is not None:
        dash = player.dashboard
        print("[Vision] 👁️  Automatically launching native Camera app for camera query")
        dash.bridge.ui_control.emit("camera_visor", "open")
        # Give the native Camera app 0.5 seconds to launch and warm up before we capture
        time.sleep(0.5)
